...
evaluation_only = False # 若为 True，脚本在第一次评估结束后立刻退出
always_save_checkpoint = True # 若为 True，每次评估后都保存一个检查点
initialize_from = 'scratch' # 可选 'scratch'（从零开始）、'resume'（续训）或 'gpt2*'
# wandb 日志记录
# wandb logging
...
maximum_iterations = 600000 # 训练迭代的总次数
weight_decay = 1e-1
beta1 = 0.9
beta2 = 0.95
gradient_clip_value = 1.0 # 把梯度裁剪到这个值；等于 0.0 表示不裁剪
# 学习率衰减相关设置
# learning rate decay settings
decay_learning_rate = True # 是否对学习率做衰减
warmup_iterations = 2000 # 预热（warmup）多少步
learning_rate_decay_iterations = 600000 # 按 Chinchilla 的建议，应该 ≈ maximum_iterations
minimum_learning_rate = 6e-5 # 最小学习率，按 Chinchilla 的建议应该 ≈ learning_rate/10
# DDP 相关设置
# DDP settings
...
data_type = 'bfloat16' if torch.cuda.is_available() and torch.cuda.is_bf16_supported() else 'float16' # 可选 'float32'、'bfloat16' 或 'float16'，最后一个会自动启用 GradScaler
